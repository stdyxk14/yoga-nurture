...
import { ScheduleStudentPicker } from "@/components/yoga/schedule-student-picker";
import {
  WorkspaceAction,
  WorkspaceActionBar,
  WorkspaceEmptyState,
...
export function ScheduleForm({ plans, students, initialPlanId, initialDate, schedule, action = createScheduleAction, mode = "create" }: Props) {
  const [state, formAction, pending] = useActionState(action, initialState);
  const initialSelectedPlanId = schedule
  ...
  return (
    <form action={formAction} aria-busy={pending} className="mx-auto max-w-[1320px] space-y-4 pb-10">
      <WorkspacePageHeader
        ...
        description={isEdit ? "日時・場所、レッスンプラン、参加予定生徒を更新します。" : "日時・場所から決め、プランが未確定でも予定を保存できます。"}
        backLink={{ href: cancelHref, label: isEdit ? "予定詳細へ戻る" : "レッスンカルテへ戻る" }}
        eyebrow="LESSON SCHEDULE"
        meta={<span>日時・場所 → レッスンプラン → 参加予定生徒</span>}
      />
      ...
      <div className="grid min-w-0 gap-4 lg:grid-cols-[minmax(0,1.08fr)_minmax(340px,0.92fr)]">
        <WorkspaceFormSection title="1. 日時・場所" description="開始と終了を分けて登録できます。">
          <div className="grid gap-4 sm:grid-cols-2">
            <WorkspaceField label="日付" required>
              <Input name="date" type="date" defaultValue={startsAt?.date ?? normalizeInitialDate(initialDate, defaultDate)} required className="yn-control" />
            </WorkspaceField>
        ...
        <WorkspaceFormSection title="2. 使用するレッスンプラン" description="後で決める場合は「未確定」のまま保存できます。">
          <div className="flex flex-wrap items-center justify-between gap-2 rounded-lg bg-[var(--yn-surface-muted)] px-3 py-2.5 text-[13px] text-[var(--yn-text-muted)]">
            <span>選択中：<strong className="font-semibold text-[var(--yn-text)]">{selectedPlan?.name ?? "未確定"}</strong></span>
          ...
          <div className="max-h-[430px] space-y-2 overflow-y-auto overscroll-contain pr-1">
            <button
              type="button"
              aria-pressed={!selectedPlanId}
              onClick={() => setSelectedPlanId("")}
              className={!selectedPlanId
                ? "flex w-full items-start gap-3 rounded-xl border border-dashed border-[#c7aa75] bg-[#fff8e8] p-3 text-left shadow-[0_3px_10px_rgba(132,102,48,0.08)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--yn-focus)]"
                : "flex w-full items-start gap-3 rounded-xl border border-dashed border-[#d8c9ad] bg-[#fffdf8] p-3 text-left transition hover:border-[#c7aa75] hover:bg-[#fff9ed] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--yn-focus)]"}
            >
              <span className={!selectedPlanId ? "mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#b28a4f] text-white" : "mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-[#decdae] bg-white text-[#9c835c]"}>
                {!selectedPlanId ? <CheckCircle2 className="h-4 w-4" aria-hidden="true" /> : <FileText className="h-4 w-4" aria-hidden="true" />}
              </span>
              <span>
                <span className="block text-[14px] font-semibold text-[#5f4d31]">未確定</span>
                <span className="mt-1 block text-[13px] text-[#806c4d]">レッスンプランは後で決める</span>
              </span>
            </button>
            {plans.map((plan) => {
          ...
          {!plans.length ? (
            <WorkspaceEmptyState
              title="保存済みのレッスンプランはありません"
              description="予定は未確定のまま保存し、あとからプランを作成して設定できます。"
      ...
      <WorkspaceFormSection title="3. 参加予定生徒" description="予定時点の参加者を選びます。未選択でも保存できます。">
        <ScheduleStudentPicker
          initialStudents={students.map(({ id, name, kana }) => ({ id, name, kana }))}
          initialSelectedIds={schedule?.participants.map((student) => student.id) ?? []}
        />
      </WorkspaceFormSection>

      <WorkspaceFormSection title="4. 当日の注意・補足" description="この予定にだけ必要な情報です。レッスンプラン本体は変更しません。">
...
function normalizeInitialDate(value: string | undefined, fallback: string) {
