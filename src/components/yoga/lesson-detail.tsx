  ...
  return (
    <>
      <div className="md:hidden">
        <MobileLessonDetail lesson={lesson} blocks={blocks} plannedStudents={plannedStudents} attended={attended} canceled={canceled} noShow={noShow} />
      </div>
    ...
    <div className="mx-auto max-w-[430px] space-y-4 overflow-x-hidden">
      <section className="rounded-[24px] border border-[#eee4d8] bg-white/84 p-4 shadow-[0_12px_26px_rgba(122,104,80,0.08)]">
        <div className="mb-2 flex items-start justify-between gap-3">
          <h1 className="min-w-0 break-words text-[22px] font-extrabold leading-tight">{lesson.title}</h1>
          <StatusBadge status={lesson.status} />
        </div>
        <div className="grid gap-2">
          <Info label="日時" value={`${lesson.date} ${lesson.startTime}-${lesson.endTime}`} />
        ...
        <p className="mt-3 break-words text-[13px] font-medium leading-6 text-[#50584e]">{lesson.theme}</p>
        <div className="mt-3 flex flex-wrap gap-1.5">
          {lesson.tags.map((tag) => <Pill key={tag}>{tag}</Pill>)}
        </div>
      </section>
      ...
      <section className="grid grid-cols-2 gap-2">
        <Link href="/lessons?tab=plans" className="inline-flex h-11 items-center justify-center rounded-2xl border border-[#d8e3d4] bg-white text-[13px] font-bold text-[#4f7b58]">一覧に戻る</Link>
        <Link href={`/lessons/${lesson.id}/script`} className="inline-flex h-11 items-center justify-center gap-1.5 rounded-2xl border border-[#cfe1ca] bg-[#f8fcf6] text-[13px] font-bold text-[#5d956d]"><Printer className="h-4 w-4" />原稿</Link>
        <Link href={`/lessons/${lesson.id}/record`} className="inline-flex h-11 items-center justify-center rounded-2xl border border-[#e7c8bd] bg-[#fff4ef] text-[13px] font-bold text-[#d96c55]">記録を書く</Link>
        <Link href={`/lessons/${lesson.id}/edit`} className="inline-flex h-11 items-center justify-center rounded-2xl bg-[#5d956d] text-[13px] font-bold text-white">編集する</Link>
      </section>

      <MobileSection title="出席サマリー">
      ...
      <MobileSection title="使用ブロック一覧">
        <div className="grid gap-3">
          {blocks.map((block, index) => (
            <Link key={block.id} href={`/blocks/${block.id}`} className="block rounded-2xl border border-[#eee4d8] bg-white/76 p-3">
              <div className="mb-2 flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="break-words text-[14px] font-extrabold">{block.name}</h3>
      ...
      <MobileSection title="参加予定生徒 / 実際の参加者">
        <div className="grid gap-2">
          {plannedStudents.map((student) => (
            <Link key={student.id} href={`/students/${student.id}`} className="block rounded-2xl border border-[#eee4d8] bg-white/76 p-3">
              <p className="text-[13px] font-extrabold">{student.name}</p>
              <p className="mt-1 line-clamp-2 text-[12px] font-medium text-[#6b7468]">{student.caution}</p>
            </Link>
          ))}
        </div>
      </MobileSection>

      <MobileSection title="レッスン後記録">
        <div className="grid gap-2">
          <Info label="全体メモ" value={lesson.actualContent || "レッスン後に追記します"} />
          <Info label="反応・観察" value={lesson.reaction || "ブロックごとに評価します"} />
          <Info label="次回改善" value={lesson.improvement || "改善点を次回プランへ反映します"} />
        </div>
        <Link href={`/lessons/${lesson.id}/record`} className="mt-3 inline-flex h-11 w-full items-center justify-center rounded-2xl bg-[#ef6f5b] text-[13px] font-bold text-white">レッスン後の記録を書く</Link>
      </MobileSection>
    </div>
  );
}
