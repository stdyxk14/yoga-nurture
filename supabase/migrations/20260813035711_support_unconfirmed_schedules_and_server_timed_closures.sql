-- Allow schedules to remain plan-unconfirmed while preserving the existing
-- atomic participant and plan-snapshot write boundary.
...
begin
  ...
  if p_schedule_id is null then
    ...
    values (
      ...
      p_lesson_plan_id,
      case
        when p_lesson_plan_id is null then 'レッスン内容未確定'
        else coalesce(nullif(btrim(p_lesson_name), ''), v_plan.name)
      end,
      p_starts_at,
      p_ends_at,
    ...
    set lesson_plan_id = p_lesson_plan_id,
        lesson_name = case
          when not v_plan_changed then lesson_name
          when p_lesson_plan_id is null then 'レッスン内容未確定'
          else coalesce(nullif(btrim(p_lesson_name), ''), v_plan.name)
        end,
...
-- Keep the existing RPC signature for deployed clients. The decision timestamp is
-- now server-managed: inserts use now(), and edits never rewrite the stored value.
create or replace function public.save_schedule_closure(
  p_schedule_id uuid,
  p_reason_code text,
...
begin
  ...
  if p_reason_code not in (
    ...
    raise exception using errcode = '22023', message = '[YN_CLOSURE_INVALID] A closure reason is required.';
  end if;
...
revoke execute on function public.save_schedule_closure(uuid, text, timestamptz, text, text, boolean) from public, anon;
