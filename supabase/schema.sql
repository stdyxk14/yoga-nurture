...
create table if not exists public.students (
  ...
  kana text,
  age_group text,
  gender text check (gender in ('女性', '男性', 'その他', '回答しない')),
  experience text,
  caution text,
  memo text,
...
create table if not exists public.student_observation_notes (
  ...
  lesson_title text,
  attendance_status text not null default '参加' check (attendance_status in ('参加', 'キャンセル', '無断欠席')),
  condition text,
  memo text,
  next_follow text,
  observed_on date,
...
create table if not exists public.lesson_plans (
  ...
  format text check (format in ('パーソナル', 'グループ', 'オンライン')),
  memo text,
...
create table if not exists public.schedules (
  ...
  place text,
  format text check (format in ('パーソナル', 'グループ', 'オンライン')),
  status text not null default '予定' check (status in ('予定', '事前準備中', '事前準備済み', '記録待ち', '記録済み')),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
create table if not exists public.lesson_record_blocks (
  ...
  reaction text check (reaction in ('良かった', '普通', 'いまいち')),
  teacher_memo text,
